...
func isLessThan(n) -> bool:
	...
	Big.normalize(n)
	Big.normalize(self)
	# Handle negative numbers robustly
	if mantissa < 0 and n.mantissa >= 0:
		return true
	if mantissa >= 0 and n.mantissa < 0:
		return false
	# Both positive or both negative, compare normally
	if exponent < n.exponent:
		return true if mantissa >= 0 else false
	elif exponent > n.exponent:
		return false if mantissa >= 0 else true
	else:
...
func isLessThanOrEqualTo(n) -> bool:
	...
	if isLessThan(n):
		return true
	if exponent == n.exponent and is_equal_approx(mantissa, n.mantissa):
		return true
	return false
